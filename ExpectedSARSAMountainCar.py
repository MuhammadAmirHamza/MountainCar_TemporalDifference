...
class ExpectedSARSA:
    ...
    def PolicyEvaluation(self):
        ...
        while True:
            ...
            innerSum = 0
            for nextAction in range(len(self.policy[nextState])):
                innerSum = innerSum + self.policy[nextState, nextAction] * self.qValueFunction[nextState, nextAction ] 
            # print(innerSum)               
            self.qValueFunction[state, action] += self.alpha * (reward + self.gamma * innerSum - self.qValueFunction[state, action])
...
obj = ExpectedSARSA(env, maxIterations = 1500)
obj.PolicyIteration()
# for i in range(obj.maxStates):
#     print(i, obj.qValueFunction[i])
