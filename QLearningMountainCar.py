...
# as we are not able to find a behavior policy that can incorporate the terminal state therefore we will implement a on policy version
# Q learning

class QLearning:
    def __init__(self, env, discount = 1, stepSize = 0.1, maxIterations = 1000):
        self.env = env
        self.gamma = discount
        self.alpha = stepSize
        self.epsilon = 0.1
    ...
    def PolicyEvaluation(self):
        ...
        while True:
            ...
            if truncated or terminated:
                break
            self.qValueFunction[state, action] += self.alpha * (reward + self.gamma * np.max(self.qValueFunction[nextState, :]) - self.qValueFunction[state, action])
    ...
    def PolicyIteration(self):
        ...
        for i in range(self.maxIterations):
            ...
            if terminated:
                terminatedCount += 1
        print('Terminated counts : ', terminatedCount)
    ...
    def testPolicy(self):
        ...
        # cummulative reward = -251

env = gym.make('MountainCar-v0', max_episode_steps = 400)
obj = QLearning(env, maxIterations = 1500)
obj.PolicyIteration()
obj.testPolicy()
